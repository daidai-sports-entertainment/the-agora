...
export function ExportModal({ isOpen, onClose, selectedNode, language, allNodes }) {
  ...
  const generatePreview = async () => {
    ...
    try {
      ...
      gradient.addColorStop(0, '#100c1c');
      gradient.addColorStop(0.5, '#18132a');
      ...
      // Elegant watermark
      const watermarkText = language === 'zh' ? '由 Agora 生成' : 'Generated by Agora';
      ctx.fillStyle = 'rgba(157, 120, 247, 0.5)';
  ...
  const drawCanvasVisualization = (ctx, width, height, selectedNode, allNodes) => {
    ...
    // Transform functions: main canvas coordinates → export canvas coordinates (centered)
      ...
      node.relationships?.forEach(rel => {
        ...
        if (targetNode && relatedNodeIds.has(targetNode.id)) {
          ...
          if (node.id === selectedNode.id || targetNode.id === selectedNode.id) {
            ...
            ctx.strokeStyle = 'rgba(157, 120, 247, 0.5)';
            ctx.lineWidth = 4;
            ctx.beginPath();
            ctx.moveTo(x1, y1);
            ctx.lineTo(x2, y2);
  ...
  const drawDetailsPanel = (ctx, leftPanelWidth, panelWidth, height, selectedNode, allNodes, language, t, padding) => {
    ...
    ctx.shadowColor = 'rgba(157, 120, 247, 0.3)';
    ctx.shadowBlur = 10;
    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 40px Georgia, serif';
    ctx.textAlign = 'left';
    const titleHeight = wrapText(ctx, selectedNode.name, xStart, yPos, panelWidth - 2 * padding, 52);
    ctx.shadowBlur = 0;
    yPos += titleHeight + 28;
    ...
    ctx.fillText('📅 ' + era, xStart, yPos);

    // Underline for era
    const eraTextWidth = ctx.measureText('📅 ' + era).width;
    ctx.fillStyle = 'rgba(157, 120, 247, 0.4)';
    ctx.fillRect(xStart, yPos + 5, eraTextWidth, 2);
    yPos += 50;

    // Description with quotation style
    ctx.fillStyle = 'rgba(157, 120, 247, 0.6)';
    ...
    if (selectedNode.key_figures && selectedNode.key_figures.length > 0) {
      ...
      ctx.fillText('👤 ' + t.keyFigures, xStart, yPos);
      yPos += 35;

      selectedNode.key_figures.slice(0, 3).forEach((figure) => {
        ctx.fillStyle = '#d4d4d4';
        ctx.font = '17px system-ui';
        ctx.fillText('• ' + figure, xStart + 10, yPos);
        yPos += 28;
      });
      yPos += 20;
    }
    ...
    ctx.textAlign = 'left';
    ctx.fillText(`🔗 ${language === 'zh' ? '关系网络' : 'Relationship Network'} (${totalRel})`, xStart, yPos);
    ...
    if (outgoing.length > 0) {
      ...
      ctx.fillText(`📤 ${language === 'zh' ? '影响' : 'Influences'} (${outgoing.length})`, xStart, yPos);
      yPos += 30;

      ctx.fillStyle = '#d4d4d4';
      ctx.font = '16px system-ui';
      outgoing.slice(0, 3).forEach(rel => {
        const text = formatRelationType(rel.type, language) + ' → ' + formatNodeName(rel.target);
        wrapText(ctx, '  • ' + text, xStart + 10, yPos, panelWidth - 2 * padding - 10, 28);
        yPos += 28;
      });
    ...
    if (incoming.length > 0) {
      ...
      ctx.font = 'bold 19px system-ui';
      ctx.fillText(`📥 ${language === 'zh' ? '被影响' : 'Influenced By'} (${incoming.length})`, xStart, yPos);
      yPos += 30;

      ctx.fillStyle = '#d4d4d4';
      ctx.font = '16px system-ui';
      incoming.slice(0, 3).forEach(rel => {
        const text = formatRelationType(rel.type, language) + ' ← ' + rel.sourceName;
        wrapText(ctx, '  • ' + text, xStart + 10, yPos, panelWidth - 2 * padding - 10, 28);
        yPos += 28;
      });
          ...
          {isGenerating ? (
            <div style={styles.loading}>
              ...
              <p style={styles.loadingText}>
                {language === 'zh' ? '生成预览中...' : 'Generating preview...'}
              </p>
            </div>
          ) : previewImage ? (
            ...
            <div style={styles.error}>
              {language === 'zh' ? '预览生成失败' : 'Failed to generate preview'}
            </div>
          )}
        </div>
...
const styles = {
  ...
  spinner: {
    width: '48px',
    height: '48px',
    border: '4px solid rgba(157, 120, 247, 0.2)',
